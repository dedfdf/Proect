...
# пауза
def pause(screen, width, height, clock):
    ...
    names = ['Продолжить', 'Настройки', 'Выход']
    version = installation_buttons(names, width, height, d_x, size_y)
    funcs = [(0,), (1, settings), (2,)]
    add_funcs(funcs, version)
    ...
    surf_border = pygame.Surface((s_dis * 3.5, size_y * 6 + s_dis * 2.5), pygame.SRCALPHA)
    pygame.draw.rect(surf_border, 'white',
                     (0, 0, s_dis * 3.5, size_y * 6 + s_dis * 2.5), width=4, border_radius=8)
    pygame.draw.rect(surf_border, (0, 0, 0, 0),
                     (s_dis * 3.5 / 2 - size_txt[0] * 0.75, 0,
    ...
    while True:
        ...
        screen.blit(surf_border, (d_x - s_dis * 1.75, y - s_dis * 1.75))

        screen.blit(text, (d_x - size_txt[0] / 2, y - s_dis * 1.75 - size_txt[1] / 2))
