...
class ButtonImage:
    def __init__(self, pos, image, angle, type, color=0):
        ...
        self.im = pygame.transform.scale(self.im, (self.im.get_width() * PERCENT_SCALE,
                                                   self.im.get_width() * PERCENT_SCALE))

        self.lock = load_image('zam.jpg', 'all_screens/image')
        self.lock = pygame.transform.scale(self.lock, (self.lock.get_width() * PERCENT_SCALE,
                                                       self.lock.get_height() * PERCENT_SCALE))

        self.size = (self.im.get_width(), self.im.get_height())
        self.pos = pos
        self.angle = angle
        self.type = int(type)
