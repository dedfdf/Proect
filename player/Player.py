...
class Player(pygame.sprite.Sprite):
    ...
    def draw_icon(self, screen):
        ...
        for i in range(self.armor):
            x = self.rect.centerx - self.w / 2 + s_border * (i + 1) + image.get_width() / 2 * i
            y = self.rect.centery - self.h / 2 + image_health.get_height() + s_border + image.get_height() / 2
            screen.blit(image, (x, y))

        weapon = self.weapon.weapon
        image = dict_weapons_icon[weapon.name]
        x = self.rect.centerx - self.w / 2 + s_border
        y = self.rect.centery + self.h / 2 - image.get_height() - s_border
        screen.blit(image, (x, y))

        size = 60 * PERCENT_SCALE
        x = self.rect.centerx - self.w / 2 + image.get_width() + s_border + size
        ...
        if weapon.name not in ['knife', 'flashlight']:
            ...
            image = infinity
            screen.blit(image, (x, y - image.get_height() / 4))
